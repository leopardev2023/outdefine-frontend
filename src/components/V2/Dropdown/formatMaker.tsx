const makeFormats = ({ selectedIndex, open }: IIClassMaker): IFormats => {
  var buttonClass =
    'relative w-full h-12 cursor-pointer rounded-lg pl-4 pr-14 text-left text-xs border-[1px] transition-all duration-150 overflow-hidden whitespace-nowrap ';
  var marker = (
    <svg
      width='16'
      height='16'
  ...
  if (selectedIndex === undefined) {
    buttonClass += open
      ...
      : ' border-[#F0F1F2] text-dark-gray ';

    // gray text color for placeholder
    buttonClass += ' bg-[#F0F1F2] ';
    return { button: buttonClass, marker };
  }

  // has selected item
  buttonClass += ' bg-lighter-gray border-odf ';

  return { button: buttonClass, marker };
};

export default makeFormats;
