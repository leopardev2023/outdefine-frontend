const makeFormats = ({
  selectedValues,
  open,
}: IMultiDropdownClassMakerFor): IFormats => {
  var buttonClass =
    'relative w-full h-12 cursor-pointer rounded-lg pl-4 pr-14 text-left text-xs border-[1px] transition-all duration-150 ';
  var marker = (
    <svg
      width='16'
      height='16'
  ...
  if (selectedValues.length === 0) {
    buttonClass += open
      ...
      : ' border-lighter-gray hover:border-dark-gray text-dark-gray ';
  ...
  // has selected item
  buttonClass += ' bg-lighter-gray border-odf text-black';
  return { button: buttonClass, marker };
};
