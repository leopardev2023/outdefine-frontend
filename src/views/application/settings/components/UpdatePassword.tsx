...
const UpdatePassword: React.FC = (): ReactElement => {
	...
	const handleSaveClick = async (event: any) => {
		...
		if (password !== confirmPassword) {
			toast.custom(<Toast type="error" message="Password did not match" />);
      		setError(true);
      		return;
    	}
	    try {
	...
	return (
    	<div className='mt-[62px] w-full bg-white rounded-lg shadow-card p-[18px_56px_16px_36px] relative'>
			...
			<div className='flex h-full w-full py-6 text-lg'>
		        ...
		        <div className={`text-sm px-6 bg-white/0 w-full font-poppins font-semibold shadow-none rounded-none border-0 ${!open ? 'border-b-[1px]': ''}`} >
